...
namespace
{
  ...
  private:
    ...
    fastuidraw::StrokedPath::SubsetSelection m_selection;
...
get_src(WriteState *state, unsigned int *chunk) const
{
  ...
  switch (drawing_mode)
    {
    case drawing_edges:
      src = &m_selection.source()->subset(m_selection.subset_ids()[idx]).painter_data();
      *chunk = 0;
      break;
...
set_join_chunks(fastuidraw::c_array<const unsigned int> subset_ids)
{
  ...
  m_join_chunks.clear();
  if (!m_join_attribute_data)
    {
      return;
    }

  for (unsigned int I : subset_ids)
    {
      StrokedPath::Subset S(m_selection.source()->subset(I));
      int J(S.join_chunk());
                  ...
                  fastuidraw::BoundingBox<float> *cvg_normalized_rect)
{
  ...
  /* Sighs. Notice that the chunks are walked in reverse order;
  ...
  c_array<const unsigned int> subsets(m_selection.subset_ids());
  m_z_increments[drawing_edges].resize(subsets.size());
  for (int i = subsets.size() - 1; i >= 0; --i)
    {
      range_type<int> R(path.subset(subsets[i]).painter_data().z_range(0));
      ...
      if (m_unused_buffers.empty())
        {
          ...
          packer = FASTUIDRAWnew PainterPacker(d->m_default_brush_shader,
                                               ...
                                               d->m_backend_factory->painter_shader_registrar(),
               ...
               fastuidraw::BoundingBox<float> *nrect)
{
  if (m_clip_rect_state.m_all_content_culled)
    ...
    {
      path.select_subsets(m_clip_store.current(),
                          m_clip_rect_state.item_matrix(),
                          m_one_pixel_width,
                          geometry_inflation,
                          m_max_attribs_per_block,
            ...
            const PathEffect *effect)
{
  ...
  d->stroke_path(shader, draw, path,
                   ...
                   const PathEffect *effect)
{
  ...
  d->stroke_path(shader.shader(stroke_style.m_cap_style),
                 draw, path,
                 number_cap_styles,
                 stroke_style.m_join_style,
                 apply_shader_anti_aliasing,
